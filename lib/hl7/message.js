...
message.prototype.log = function() {
  var currentSeparator = this.header.delimiters.segmentSeparator
  this.header.delimiters.segmentSeparator = '\n';
  var returnString = this.toString();
  this.header.delimiters.segmentSeparator = currentSeparator;
  return returnString
...
message.prototype.toString = function() {
  var returnString = this.header.toString() + this.header.delimiters.segmentSeparator;

  for (var i = 0; i < this.segments.length; i++) {

    returnString += this.segments[i].toString(this.header.delimiters);
    if (i != this.segments.length - 1) returnString += this.header.delimiters.segmentSeparator;
  }
